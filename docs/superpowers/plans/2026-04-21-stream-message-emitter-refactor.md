...
| bash | PowerShell 等价 |
|------|----------------|
| `./mvnw -pl skill-server test -q` | `.\mvnw.cmd -pl skill-server test -q` |
| `./mvnw -pl skill-server compile -q` | `.\mvnw.cmd -pl skill-server compile -q` |
...
**⚠️ 先决任务**：Task 10/13 会给 `GatewayMessageRouter` / `GatewayRelayService` 追加构造器参数 `StreamMessageEmitter emitter`，这将破坏仓库里 5 个手动 `new` 受影响类的测试文件的编译，以及 20+ 处 `verify(outboundDeliveryDispatcher).deliver(...)` / `verify(bufferService).accumulate(...)` 断言。**必须先完成 Task 9.5** 统一适配，否则 Phase 2 结束时测试无法通过。
...
**背景**：受影响的 5 个测试文件都手动 `new` Router/RelayService 实例（而非 `@InjectMocks`），并对 `outboundDeliveryDispatcher` / `bufferService` 做断言。Phase 2 的 Router 改造会：
- 让 `new GatewayMessageRouter(...)` 构造器参数多出 `StreamMessageEmitter emitter`
- 让 Router 内部 "enrich + dispatcher.deliver" 改成 `emitter.emitToSession(...)`，此后 dispatcher / bufferService 不再被 Router 直接调用

因此：
- 所有 `verify(outboundDeliveryDispatcher).deliver(any(), eq("xxx"), ..., capture)` → 改成 `verify(emitter).emitToSession(any(), eq("xxx"), ..., capture)`
- 所有 `verify(bufferService).accumulate(eq("xxx"), any())` → 改成 `verify(emitter).emitToClientWithBuffer(eq("xxx"), any())`
- `verify(persistenceService).persistIfFinal(...)` **保留不变**（该方法不在 enrich 路径内，Router 独立调用，迁移不影响）
...
| `verify(bufferService).accumulate(eq("..."), any(StreamMessage.class))` | `verify(emitter).emitToClientWithBuffer(eq("..."), any(StreamMessage.class))` |

**行号提示**（如仓库结构未变）：
- dispatcher 断言：line 134, 160, 184, 218, 230, 253, 274, 295, 350, 375, 398, 477, 492, 528, 541, 553, 581, 602, 637, 673, 699（共 **21 处**）
- bufferService 断言：line 137, 233
- persistIfFinal 断言：line 138, 162, 234, 641 — **保持不变**
...
> sed -i 's/verify(outboundDeliveryDispatcher, times/verify(emitter, times/g; s/\.deliver(any(), /\.emitToSession(any(), /g' \
>     skill-server/src/test/java/com/opencode/cui/skill/service/GatewayRelayServiceTest.java
> ```
> 执行后人工 review 一次 diff，因为某些调用的 captor 变量名（msgCaptor）也要在 assertion 后保持正确。
...
- [ ] **Step 6: 检查其他文件是否也有需迁的断言**

```bash
grep -rn "verify(outboundDeliveryDispatcher\|verify(bufferService).accumulate" skill-server/src/test
```

如果 Step 4/5 之后还有命中，按同一规则替换。
