...
`feison` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/feison/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/feison/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee feison` to
...
python ./.trellis/scripts/task.py archive 00-join-feison
```
