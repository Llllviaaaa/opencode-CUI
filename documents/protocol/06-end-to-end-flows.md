...
- **消息格式：**
  ...
  ```
- **字段来源：** `deviceName` 来自系统 hostname，`macAddress` 来自网卡，`os` 来自 `process.platform`，`toolType` 来自配置 `gateway.channel`，`toolVersion` 来自 `global.health().version`
...
- **组件：** Plugin (BridgeRuntime)
- **方法：** `BridgeRuntime.handleEvent()` → `upstreamTransportProjector.project()` → `gatewayConnection.send({type: "tool_event", toolSessionId, event})`
- **处理步骤：**
  1. `UpstreamEventExtractor.extract(rawEvent)` — 提取 common 和 extra 信息
  2. 检查连接状态为 READY
  3. 检查事件在 allowlist 中
  4. 构建 `tool_event` 消息：默认透传原始事件；`message.updated` 会投影为轻量结构
- **存储操作：** 无
...
note left: question.asked\nproperties.id + tool.callID\nquestions[0]: question, options, header
...
activate PL
PL -> OC : 查找并回答问题 [S4.9]
note right: Step1: GET /question\nStep2: 筛选 sessionID + callID\nStep3: POST /question/{requestID}/reply\nrequestID 来源于列表项 id

PL -> PL : question_reply 成功完成\n不发送 tool_done [S4.10]
deactivate PL
...
- **事件格式：**
  ...
  ```
- **说明：** `questions` 是数组但通常只有一个元素；`question_reply` 时请求标识从 `GET /question` 返回项的 `id` 字段获取
- **存储操作：** 无
...
- **处理步骤：**
  ...
  5. `POST /question/{requestID}/reply` → `{answers: [["是"]]}`（`requestID` 的值取自列表项 `id`）
- **说明：** `answers` 是二维数组，第一层对应多个问题，第二层对应多个答案选项
- **存储操作：** 无

#### [S4.10] Plugin 完成 question_reply（不发送 tool_done）

- **组件：** Plugin (BridgeRuntime)
- **方法：** `QuestionReplyAction.execute()` 成功返回；`ToolDoneCompat.handleInvokeCompleted(action='question_reply')` 返回 `emit=false`
- **说明：** question_reply 成功后默认不发送 `tool_done`；仅当后续出现 `session.idle` 且命中兜底条件时，才可能发送 `tool_done`
- **存储操作：** 无
...
#### [S5.10] Plugin 完成 permission_reply（不发送 tool_done）

- **组件：** Plugin (BridgeRuntime)
- **方法：** `PermissionReplyAction.execute()` 成功返回；`ToolDoneCompat.handleInvokeCompleted(action='permission_reply')` 返回 `emit=false`
- **说明：** permission_reply 成功后默认不发送 `tool_done`；仅当后续出现 `session.idle` 且命中兜底条件时，才可能发送 `tool_done`
- **存储操作：** 无
...
- **组件：** Plugin (BridgeRuntime)
- **方法：** `BridgeRuntime.handleEvent()` → `upstreamTransportProjector.project()` → `gatewayConnection.send({type: "tool_event", event})`
...
#### [S10.7] Plugin 完成 close_session（不发送 tool_done）

- **组件：** Plugin (BridgeRuntime)
- **方法：** `CloseSessionAction.execute()` 成功返回；`ToolDoneCompat.handleInvokeCompleted(action='close_session')` 返回 `emit=false`
- **说明：** close_session 成功后默认不发送 `tool_done`；仅当后续出现 `session.idle` 且命中兜底条件时，才可能发送 `tool_done`
- **存储操作：** 无
...
#### [S11.6] Plugin 完成 abort_session（不发送 tool_done）

- **组件：** Plugin (BridgeRuntime)
- **方法：** `AbortSessionAction.execute()` 成功返回；`ToolDoneCompat.handleInvokeCompleted(action='abort_session')` 返回 `emit=false`
- **说明：** abort_session 成功后默认不发送 `tool_done`；仅当后续出现 `session.idle` 且命中兜底条件时，才可能发送 `tool_done`
- **存储操作：** 无

#### [S11.7] Skill Server 等待后续事件驱动会话收敛

- **组件：** Skill Server (GatewayMessageRouter)
- **方法：** 由后续 `tool_event` / `session.idle` 路径驱动状态收敛（同场景 6）
- **说明：** abort 操作本身不会直接触发 `tool_done`，会话是否进入 idle 取决于后续 OpenCode 事件
- **存储操作：** 依赖后续事件处理路径

#### [S11.8] Miniapp 恢复正常状态（事件驱动）

- **组件：** Miniapp (useSkillStream)
- **方法：** 接收后续 `session.status=idle` 后 `finalizeAllStreamingMessages()`
- **与关闭会话的区别：**
...
## 场景 15：Agent 健康查询（status_query）

Gateway 对在线 Agent 发起健康查询，Plugin 返回 OpenCode 在线状态。

### 时序图

```plantuml
@startuml 场景15_Agent健康查询
skinparam style strictuml
autonumber
...
MA -> SS : 查询 Agent 状态 [S15.1]
SS -> GW : 请求 Agent 健康状态 [S15.2]

GW -> PL : status_query（非 invoke） [S15.3]
activate PL
PL -> OC : global.health() [S15.4]
OC --> PL : {healthy, version}
PL -> GW : status_response {opencodeOnline} [S15.5]
deactivate PL

GW -> SS : 返回健康状态 [S15.6]
SS -> MA : Agent 在线状态 [S15.7]
...
#### [S15.1] Miniapp 查询 Agent 状态

- **组件：** Miniapp
- **方法：** 触发状态查询请求（页面加载 / 手动刷新）
- **存储操作：** 无

#### [S15.2] Skill Server 请求 Gateway 健康状态

- **组件：** Skill Server
- **方法：** 调用 Gateway 的 Agent 状态查询接口
- **存储操作：** 无

#### [S15.3] Gateway 发送 status_query 到 Plugin
...
- **消息格式：**
  ...
  { "type": "status_query" }
  ```
- **说明：** `status_query` 是独立消息类型，不属于 `invoke.action`
- **存储操作：** 无

#### [S15.4] Plugin 执行健康检查

- **组件：** Plugin (StatusQueryAction)
- **方法：** `hostClient.global.health()`
- **判定规则：** `healthy === true` 视为在线；异常或其它值视为离线
- **存储操作：** 无

#### [S15.5] Plugin 返回 status_response

- **组件：** Plugin → AI Gateway
- **消息格式：**
  ```json
...
#### [S15.6] Gateway 返回状态到 Skill Server

- **组件：** AI Gateway
- **方法：** 汇总 `status_response` 后回传调用方
- **存储操作：** 无

#### [S15.7] Skill Server 推送状态给 Miniapp

- **组件：** Skill Server → Miniapp
- **结果：** 前端更新 Agent 在线/离线状态
- **存储操作：** 无
