 ...
 * 以及从 {@link GatewayMessage} 批量提取字段到 MDC 的能力。</p>
 *
 * <h3>使用方式</h3>
 * <pre>{@code
 * // WebSocket handler 入口
 * try {
 *     MdcHelper.fromGatewayMessage(message);
 *     MdcHelper.putScenario("ws-agent-invoke");
 *     // ... 业务逻辑
 * } finally {
 *     MdcHelper.clearAll();
 * }
...
public final class MdcHelper {
    ...
    /**
     * 从 GatewayMessage 中提取 traceId、welinkSessionId、ak、userId 到 MDC。
     *
     * @param msg GatewayMessage 实例，null 安全
     */
    public static void fromGatewayMessage(GatewayMessage msg) {
        if (msg == null) {
            return;
        }
        safePut(MdcConstants.TRACE_ID, msg.getTraceId());
        safePut(MdcConstants.SESSION_ID, msg.getWelinkSessionId());
        safePut(MdcConstants.AK, msg.getAk());
        safePut(MdcConstants.USER_ID, msg.getUserId());
    }
     ...
     * 用于跨线程传播：在原线程 snapshot()，在目标线程 restore()。
     *
