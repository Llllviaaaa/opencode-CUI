...
class OpenCodeEventTranslatorTest {
  ...
  void translatesQuestionAsked() throws Exception {
    ...
    // questionId 与 properties.id == partId 同源（personal scope 快路径）
    assertEquals("question-1", translated.getQuestionInfo().getQuestionId());
    assertEquals(translated.getPartId(), translated.getQuestionInfo().getQuestionId());
  }

  @Test
  @DisplayName("question.asked: questionId == partId == properties.id (personal scope fast-path)")
  void questionRequestIdMatchesPartIdAndPropertiesId() throws Exception {
    ...
    assertEquals("req-uuid-abc", m.getQuestionInfo().getQuestionId());
  }

  @Test
  @DisplayName("StreamMessage @JsonUnwrapped surfaces QuestionInfo.questionId at top level")
  void questionInfoRequestIdSerializedAtTopLevel() throws Exception {
    ...
    assertEquals("req-1", node.path("questionId").asText());
